...
public:
	...
	// Writing from arrays to document, this is to update the document whenever the user reduces or increases the amount.
	...
	virtual void Decreaser(AbstractClass& Obj, int* Amount, size_t index) = 0;
	...
	Done	-	Method to increase the amount by a user input value, then changing the coresponding array value and lastly update the array to .txt file.
				One method per array index OR array index as func parameter?
	Done	-	Method to reduce the amount by a user input value, then changing the coresponding array value and lastly update the array to .txt file.
				One method per array index OR array index as func parameter?
	...
	N		-	Method for setting threshold amount.
